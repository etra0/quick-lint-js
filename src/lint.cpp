...
void linter::propagate_variable_declarations_to_parent_scope() {
  ...
  for (const auto &[_var_name, var] : current_scope.declared_variables) {
...
const linter::declared_variable *linter::scope::add_variable_declaration(
  ...
  auto it = this->declared_variables.emplace(
      name_view,
      declared_variable{
        string8(name_view), kind, name, declared_scope});
  return &it->second;
}
...
void linter::scope::add_predefined_variable_declaration(const char8 *name,
  ...
  this->declared_variables.emplace(
      name_view,
      declared_variable{
...
const linter::declared_variable *linter::scope::find_declared_variable(
  ...
  return it == this->declared_variables.end() ? nullptr : &it->second;
}
}
